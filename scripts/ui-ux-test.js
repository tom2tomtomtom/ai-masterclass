...
async function testCourseCatalog(page) {
  ...
  test('Course detail pages load and display content', async ({ page }) => {
    console.log('📖 Testing Course Detail Pages...');
    
    // Look for clickable course elements
    const courseLinks = page.locator('a[href*="course"], a[href*="module"], .course-card a, .course-item a');
    const linkCount = await courseLinks.count();
    
    if (linkCount > 0) {
      console.log(`Found ${linkCount} course links`);
      
      // Test first few course detail pages
      for (let i = 0; i < Math.min(3, linkCount); i++) {
        const courseLink = courseLinks.nth(i);
  ...
  test('Backend API connectivity and data flow', async ({ page }) => {
